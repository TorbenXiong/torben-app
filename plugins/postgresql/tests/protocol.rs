...
#[test]
fn bundled_postgresql_plugin_serves_handshake_and_schema_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_torben-plugin-postgresql"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert!(
        initialized
            .applications
            .iter()
            .any(|app| app.id.as_str() == "postgresql")
    );
    let schema: SchemaPageListResult = call(
        ...
        &SchemaPageListParams {
            plugin_id: PluginId::new("app.torben.plugin.postgresql").unwrap(),
        },
    );
    assert_eq!(schema.pages[0].id, "postgresql");
    drop(input);
