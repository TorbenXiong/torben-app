...
use torben_core::PostgresqlProvider;

const PLUGIN_ID: &str = "app.torben.plugin.postgresql";
const APP_ID: &str = "postgresql";
const SOURCE_ID: &str = "postgresql.edb";
...
async fn serve() -> Result<(), TorbenError> {
    let provider = PostgresqlProvider::official()?;
...
fn handle(provider: &PostgresqlProvider, request: JsonRpcRequest) -> JsonRpcResponse {
    let id = request.id;
    ...
    match method_name {
        method::INITIALIZE => {
                ...
                || params.target != current_target()
            {
                return Err(TorbenError::new(
                    ...
                    "The PostgreSQL plugin host protocol or target is incompatible.",
                ));
            }
        ...
        method::INSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? || params.target != current_target() {
                return Err(TorbenError::new(
                    ...
                    "The PostgreSQL install request changed source ownership or target.",
                ));
            }
            ...
            value(InstallPlan {
                ...
                steps: vec![
                    InstallStep::Download {
                        ...
                        destination_name: distribution.installer_name.clone(),
                    },
                    InstallStep::VerifySha256 {
                        archive_name: distribution.installer_name.clone(),
                        expected: distribution.checksum,
                    },
                    InstallStep::ExtractArchive {
                        archive_name: distribution.installer_name,
                        strip_components: 0,
                    },
                    InstallStep::HealthCheck {
                        executable: "postgres".to_owned(),
        ...
        method::HEALTH_CHECK => {
            ...
            value(HealthCheckResult {
                ...
                message: "PostgreSQL server, client, initialization, control, and backup commands passed health checks."
                    .to_owned(),
            })
        }
        ...
        method::UNINSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? {
                return Err(TorbenError::new(
                    ...
                    "The PostgreSQL uninstall source owner is invalid.",
                ));
            }
            value(UninstallPlan {
                app_id: params.app_id,
...
fn descriptor() -> Result<ApplicationDescriptor, TorbenError> {
    Ok(ApplicationDescriptor {
        ...
        summary: "PostgreSQL server and command-line tools from the EDB Windows distribution."
            .to_owned(),
        ...
        sources: vec![InstallSource {
            ...
            display_name: "EDB PostgreSQL Windows distribution".to_owned(),
            managed: true,
        }],
    })
}
...
fn schema_page() -> SchemaPage {
    SchemaPage {
        id: APP_ID.to_owned(),
        title: "PostgreSQL provider".to_owned(),
        description: Some(
        ...
        sections: vec![SchemaSection {
            ...
            fields: vec![
                status(
                    "source",
                    "Release source",
                    "PostgreSQL Windows installer distributed by EDB",
                ),
                status(
                    "integrity",
                    "Integrity",
                    "Installer SHA-256 pinned to the Microsoft WinGet manifest",
                ),
                SchemaField {
                    id: "target".to_owned(),
...
fn ensure_app(app_id: &AppId) -> Result<(), TorbenError> {
    if app_id.as_str() == APP_ID {
        ...
        Err(TorbenError::new(
            ...
            "The PostgreSQL plugin received a request for another application.",
        ))
    }
}
...
fn parse<T: serde::de::DeserializeOwned>(value: Value) -> Result<T, TorbenError> {
    serde_json::from_value(value).map_err(|error| {
        TorbenError::new(
            ...
            "The PostgreSQL plugin request parameters are invalid.",
        )
        .with_detail("reason", error.to_string())
    })
}
...
fn serialize_error(error: serde_json::Error) -> TorbenError {
    TorbenError::internal("The PostgreSQL plugin could not serialize a response.")
        .with_detail("reason", error.to_string())
}
fn io_error(error: std::io::Error) -> TorbenError {
    TorbenError::new(
        "plugin_io_failed",
        "The PostgreSQL plugin stdio operation failed.",
    )
    .with_detail("reason", error.to_string())
}
