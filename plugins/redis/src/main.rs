...
use torben_core::RedisProvider;

const PLUGIN_ID: &str = "app.torben.plugin.redis";
const APP_ID: &str = "redis";
const SOURCE_ID: &str = "redis.windows";
...
async fn serve() -> Result<(), TorbenError> {
    let provider = RedisProvider::windows_community()?;
...
fn handle(provider: &RedisProvider, request: JsonRpcRequest) -> JsonRpcResponse {
    let id = request.id;
    ...
    match method_name {
        method::INITIALIZE => {
                ...
                || params.target != current_target()
            {
                return Err(TorbenError::new(
                    ...
                    "The Redis plugin host protocol or target is incompatible.",
                ));
            }
        ...
        method::INSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? || params.target != current_target() {
                return Err(TorbenError::new(
                    ...
                    "The Redis install request changed source ownership or target.",
                ));
            }
            ...
            value(InstallPlan {
                ...
                steps: vec![
                    ...
                    InstallStep::HealthCheck {
                        executable: "redis-server".to_owned(),
                        arguments: vec!["--version".to_owned()],
                        expected_output: format!("Redis server v={}", params.version),
                    },
                    InstallStep::CreateShims {
                        commands: vec![
                            "redis-server".to_owned(),
                            "redis-cli".to_owned(),
                            "redis-benchmark".to_owned(),
                        ],
                    },
                ],
        ...
        method::HEALTH_CHECK => {
            ...
            value(HealthCheckResult {
                ...
                message: "redis-server, redis-cli, and redis-benchmark health checks passed."
                    .to_owned(),
            })
        }
        ...
        method::UNINSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? {
                return Err(TorbenError::new(
                    ...
                    "The Redis uninstall source owner is invalid.",
                ));
            }
            value(UninstallPlan {
                app_id: params.app_id,
...
fn descriptor() -> Result<ApplicationDescriptor, TorbenError> {
    Ok(ApplicationDescriptor {
        id: AppId::new(APP_ID)?,
        display_name: "Redis".to_owned(),
        summary: "Redis Community Server with redis-server, redis-cli, and redis-benchmark."
            .to_owned(),
        ...
        sources: vec![InstallSource {
            ...
            display_name: "Redis for Windows community build".to_owned(),
            managed: true,
        }],
    })
}
        ...
        sections: vec![SchemaSection {
            ...
            fields: vec![
                status(
                    "source",
                    "Release source",
                    "redis-windows community build from upstream Redis source",
                ),
                status(
                    "integrity",
                    "Integrity",
                    "SHA-256 pinned from the redis-windows release",
                ),
                SchemaField {
                    id: "target".to_owned(),
...
fn ensure_app(app_id: &AppId) -> Result<(), TorbenError> {
    if app_id.as_str() == APP_ID {
        ...
        Err(TorbenError::new(
            ...
            "The Redis plugin received a request for another application.",
        ))
    }
}
...
fn parse<T: serde::de::DeserializeOwned>(value: Value) -> Result<T, TorbenError> {
    serde_json::from_value(value).map_err(|error| {
        TorbenError::new(
            ...
            "The Redis plugin request parameters are invalid.",
        )
        .with_detail("reason", error.to_string())
    })
}
...
fn serialize_error(error: serde_json::Error) -> TorbenError {
    TorbenError::internal("The Redis plugin could not serialize a response.")
        .with_detail("reason", error.to_string())
}
fn io_error(error: std::io::Error) -> TorbenError {
    TorbenError::new(
        "plugin_io_failed",
        "The Redis plugin stdio operation failed.",
    )
    .with_detail("reason", error.to_string())
}
