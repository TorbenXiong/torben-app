...
#[test]
fn bundled_redis_plugin_serves_handshake_and_schema_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_torben-plugin-redis"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert_eq!(initialized.plugin_id.as_str(), "app.torben.plugin.redis");
    assert!(
        initialized
            .applications
            .iter()
            .any(|app| app.id.as_str() == "redis")
    );
    let schema: SchemaPageListResult = call(
        ...
        &SchemaPageListParams {
            plugin_id: PluginId::new("app.torben.plugin.redis").unwrap(),
        },
    );
    assert_eq!(schema.pages[0].id, "redis");
    drop(input);
