...
use torben_core::RustProvider;

const PLUGIN_ID: &str = "app.torben.plugin.rust";
const APP_ID: &str = "rust";
const SOURCE_ID: &str = "rust.official";
...
async fn serve() -> Result<(), TorbenError> {
    let provider = RustProvider::official()?;
...
async fn handle(provider: &RustProvider, request: JsonRpcRequest) -> JsonRpcResponse {
    let id = request.id;
    ...
    match method_name {
        method::INITIALIZE => {
                ...
                || params.target != current_target()
            {
                return Err(TorbenError::new(
                    ...
                    "The Rust plugin host protocol or target is incompatible.",
                ));
            }
        ...
        method::INSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? || params.target != current_target() {
                return Err(TorbenError::new(
                    ...
                    "The Rust install request changed source ownership or target.",
                ));
            }
            ...
            value(InstallPlan {
                ...
                steps: vec![
                    ...
                    InstallStep::HealthCheck {
                        ...
                        expected_output: format!("rustc {}", params.version),
                    },
        ...
        method::HEALTH_CHECK => {
            ...
            value(HealthCheckResult {
                ...
                message: "rustc, cargo, rustdoc, and rustfmt health checks passed.".to_owned(),
            })
        }
        ...
        method::UNINSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? {
                return Err(TorbenError::new(
                    ...
                    "The Rust uninstall source owner is invalid.",
                ));
            }
            value(UninstallPlan {
                app_id: params.app_id,
...
fn descriptor() -> Result<ApplicationDescriptor, TorbenError> {
    Ok(ApplicationDescriptor {
        ...
        summary: "Rust toolchains with rustc, Cargo, rustdoc, and rustfmt.".to_owned(),
        ...
        sources: vec![InstallSource {
            ...
            display_name: "Official Rust distribution".to_owned(),
            managed: true,
        }],
    })
}
...
fn schema_page() -> SchemaPage {
    SchemaPage {
        id: APP_ID.to_owned(),
        title: "Rust provider".to_owned(),
        description: Some("Official Rust stable toolchains and managed Cargo caches.".to_owned()),
        sections: vec![SchemaSection {
            id: "trust".to_owned(),
            title: Some("Supply-chain status".to_owned()),
            description: None,
            fields: vec![
                status("source", "Release source", "Rust official distribution"),
                status(
                    "integrity",
                    "Integrity",
                    "SHA-256 from the official channel manifest",
                ),
                SchemaField {
                    id: "target".to_owned(),
...
fn ensure_app(app_id: &AppId) -> Result<(), TorbenError> {
    if app_id.as_str() == APP_ID {
        ...
        Err(TorbenError::new(
            ...
            "The Rust plugin received a request for another application.",
        ))
    }
}
...
fn parse<T: serde::de::DeserializeOwned>(value: Value) -> Result<T, TorbenError> {
    serde_json::from_value(value).map_err(|error| {
        TorbenError::new(
            ...
            "The Rust plugin request parameters are invalid.",
        )
        .with_detail("reason", error.to_string())
    })
}
...
fn serialize_error(error: serde_json::Error) -> TorbenError {
    TorbenError::internal("The Rust plugin could not serialize a response.")
        .with_detail("reason", error.to_string())
}
fn io_error(error: std::io::Error) -> TorbenError {
    TorbenError::new(
        "plugin_io_failed",
        "The Rust plugin stdio operation failed.",
    )
    .with_detail("reason", error.to_string())
}
