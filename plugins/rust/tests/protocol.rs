...
#[test]
fn bundled_rust_plugin_serves_handshake_and_schema_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_torben-plugin-rust"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert_eq!(initialized.plugin_id.as_str(), "app.torben.plugin.rust");
    assert!(
        initialized
            .applications
            .iter()
            .any(|app| app.id.as_str() == "rust")
    );
    let schema: SchemaPageListResult = call(
        ...
        &SchemaPageListParams {
            plugin_id: PluginId::new("app.torben.plugin.rust").unwrap(),
        },
    );
    assert_eq!(schema.pages[0].id, "rust");
    drop(input);
