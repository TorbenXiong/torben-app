...
use torben_core::VsCodeProvider;

const PLUGIN_ID: &str = "app.torben.plugin.vscode";
const APP_ID: &str = "vscode";
const SOURCE_ID: &str = "vscode.official";
...
async fn serve() -> Result<(), TorbenError> {
    let provider = VsCodeProvider::official()?;
...
async fn handle(provider: &VsCodeProvider, request: JsonRpcRequest) -> JsonRpcResponse {
    let id = request.id;
        ...
        method::INITIALIZE => {
            ...
            if params.protocol_version != PLUGIN_PROTOCOL_VERSION {
                return Err(TorbenError::new(
                    ...
                    "The host protocol is incompatible with the Visual Studio Code plugin.",
                ));
            }
            ...
            if params.host_version < plugin_version || params.target != current_target() {
                return Err(TorbenError::new(
                    ...
                    "The Visual Studio Code plugin host version or target is incompatible.",
                ));
            }
            value(InitializeResult {
                ...
                plugin_version,
                applications: vec![vscode_descriptor()?],
            })
        }
        method::APP_DESCRIBE => value(vscode_descriptor()?),
        method::VERSIONS_LIST => {
            let params: VersionListParams = parse(params)?;
            ensure_vscode(&params.app_id)?;
        ...
        method::INSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? || params.target != current_target() {
                return Err(TorbenError::new(
                    ...
                    "The Visual Studio Code install request changed source ownership or target.",
                ));
            }
            ...
            value(InstallPlan {
                ...
                steps: vec![
                    ...
                    InstallStep::HealthCheck {
                        ...
                        arguments: vec!["--version".to_owned(), "--disable-updates".to_owned()],
                        expected_output: params.version.to_string(),
                    },
                    InstallStep::CreateShims {
                        commands: vec!["code".to_owned()],
                    },
                ],
                metadata: BTreeMap::from([
                    ("target".to_owned(), current_target()),
                    ("platform".to_owned(), distribution.platform),
                    ("commit".to_owned(), distribution.commit),
                ]),
            })
        }
        method::HEALTH_CHECK => {
            ...
            value(HealthCheckResult {
                ...
                message: "Visual Studio Code version health check passed.".to_owned(),
            })
        }
        ...
        method::UNINSTALL_PLAN => {
            ...
            if params.source_id != SourceId::new(SOURCE_ID)? {
                return Err(TorbenError::new(
                    ...
                    "The Visual Studio Code uninstall source owner is invalid.",
                ));
            }
            value(UninstallPlan {
                app_id: params.app_id,
...
fn vscode_descriptor() -> Result<ApplicationDescriptor, TorbenError> {
    Ok(ApplicationDescriptor {
        id: AppId::new(APP_ID)?,
        display_name: "Visual Studio Code".to_owned(),
        summary: "Microsoft's official cross-platform code editor distribution.".to_owned(),
        categories: vec!["editor".to_owned(), "development".to_owned()],
        capabilities: vec![
            "versions".to_owned(),
            "install".to_owned(),
            "select".to_owned(),
        ...
        sources: vec![InstallSource {
            ...
            display_name: "Microsoft Visual Studio Code archive".to_owned(),
            managed: true,
        }],
    })
}

fn vscode_schema_page() -> SchemaPage {
    SchemaPage {
        id: "vscode".to_owned(),
        title: "Visual Studio Code provider".to_owned(),
        description: Some(
        ...
        sections: vec![SchemaSection {
            ...
            fields: vec![
                status_field(
                    "source",
                    "Release source",
                    "Published microsoft/vscode release + Microsoft Update metadata",
                ),
                status_field("integrity", "Integrity", "Microsoft SHA-256 + exact commit"),
                status_field("updates", "Managed launch", "code --disable-updates"),
                SchemaField {
                    id: "target".to_owned(),
...
fn ensure_vscode(app_id: &AppId) -> Result<(), TorbenError> {
    if app_id.as_str() == APP_ID {
        Ok(())
    } else {
        Err(TorbenError::new(
            "plugin_application_mismatch",
            "The Visual Studio Code plugin received a request for another application.",
        ))
    }
}
...
fn parse<T: serde::de::DeserializeOwned>(value: Value) -> Result<T, TorbenError> {
    serde_json::from_value(value).map_err(|error| {
        TorbenError::new(
            ...
            "The Visual Studio Code plugin request parameters are invalid.",
        )
        .with_detail("reason", error.to_string())
    })
}
...
fn serialize_error(error: serde_json::Error) -> TorbenError {
    TorbenError::new(
        ...
        "The Visual Studio Code plugin could not serialize its response.",
    )
    .with_detail("reason", error.to_string())
}
...
fn io_error(error: std::io::Error) -> TorbenError {
    TorbenError::new(
        "plugin_io_failed",
        "The Visual Studio Code plugin stdio channel failed.",
    )
    .with_detail("reason", error.to_string())
}
