...
#[test]
fn bundled_vscode_plugin_serves_handshake_and_schema_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_torben-plugin-vscode"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert_eq!(initialized.plugin_id.as_str(), "app.torben.plugin.vscode");
    assert!(
        initialized
            .applications
            .iter()
            .any(|application| application.id.as_str() == "vscode")
    );
    let schema: SchemaPageListResult = call(
        ...
        &SchemaPageListParams {
            plugin_id: PluginId::new("app.torben.plugin.vscode").unwrap(),
        },
    );
    assert_eq!(schema.pages[0].id, "vscode");
    drop(input);
