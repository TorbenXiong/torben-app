...
#[test]
fn bundled_temurin_plugin_serves_handshake_and_schema_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_torben-plugin-temurin"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert_eq!(initialized.plugin_id.as_str(), "app.torben.plugin.temurin");
    assert!(
        initialized
            .applications
            .iter()
            .any(|application| application.id.as_str() == "temurin")
    );
    ...
    let schema: SchemaPageListResult = call(
        ...
        &SchemaPageListParams {
            plugin_id: PluginId::new("app.torben.plugin.temurin").unwrap(),
        },
    );
    assert_eq!(schema.pages.len(), 1);
    assert_eq!(schema.pages[0].id, "temurin");
