...
#[test]
fn bundled_git_plugin_serves_handshake_and_schema_over_stdio() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_torben-plugin-git"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    assert_eq!(initialized.plugin_id.as_str(), "app.torben.plugin.git");
    assert!(
        initialized
            .applications
            .iter()
            .any(|application| application.id.as_str() == "git")
    );
    ...
    let schema: SchemaPageListResult = call(
        ...
        &SchemaPageListParams {
            plugin_id: PluginId::new("app.torben.plugin.git").unwrap(),
        },
    );
    assert_eq!(schema.pages[0].id, "git");
